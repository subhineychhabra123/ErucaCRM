...
(function () {
    var Circles = window.Circles = function (options) {
        ...
        this._percentage = Math.abs(options.percentage);
        this._text = options.text; // #3
        this._number = options.number || options.percentage;
    ...
    Circles.prototype = {
        ...
        _calculateNumber: function (number) {
            ...
            var data = this._text == '' ? '' : parts[0];
            html = '<span class="' + this._numberClass + '">' +  data;
            if (parts.length > 1) {
                var val = this._text == '' ? '' : parts[1].substring(0, 2);
                html += '.<span style="font-size:.4em">' + val + '</span>';
            }
            return html + '</span>';
        },
