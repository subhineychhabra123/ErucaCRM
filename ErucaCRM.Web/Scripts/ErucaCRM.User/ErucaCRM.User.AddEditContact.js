//Register name space
jQuery.namespace('ErucaCRM.User.AddEditContact');
var viewModel;
var tagcnt = 0;
...
ErucaCRM.User.AddEditContact.pageLoad = function () {
    TagList = new Array();
    CurrentTagObject = new Object();
    CurrentTagObject.TagId = "";
    CurrentTagObject.TagName = "";
    viewModel = new ErucaCRM.User.AddEditContact.pageViewModel();
    ko.applyBindings(viewModel, document.getElementById("InnerContentContainer"));
    tagcnt = 0;

}
ErucaCRM.User.AddEditContact.pageViewModel = function (emailId) {
    //Class variables
    var controllerUrl = "/User/";
    var self = this;
    $("#TagIds").val("");
    self.availableTags = ["Bussiness Manager", "Potential", "IMPORTANT", "Jan", "Feb", "March", "April", "May", "June", "July", "August", "Sept", "Oct", "Nov", "Dec", "Seminar", "Webinar", "Interested", "Soon", "Contact"];
    ...
    self.RemoveTag = function (tag) {
        ...
        if (confirm(ErucaCRM.Messages.Contact.TagRemoveConfirmAction)) {
    ...
    $('#Tag').keypress(function (event) {
        ...
        if (event.keyCode == 13) {
            self.AddTagToContact();
            event.preventDefault();
        }
    });
    self.SaveTagToContact = function () {
        var TagIds = "";
        var NewTagNames = "";
